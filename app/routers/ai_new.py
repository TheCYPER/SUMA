...
async def query_ai_assistant(
    ...
    if not test_ollama_connection():
        return AIResponse(
            response="AI助手暂时不可用。请确保Ollama服务正在运行。\n\n启动步骤：\n1. 安装Ollama: https://ollama.ai\n2. 启动服务: ollama serve\n3. 下载模型: ollama pull llama3.1:8b",
            suggestions=["检查Ollama服务状态", "重新启动Ollama", "检查网络连接"]
        )
    ...
    try:
        ...
        if not guardrail_result["allowed"]:
            if guardrail_result["action"] == "intervention":
                ...
                return AIResponse(
                    ...
                    learning_tips=["负责任使用AI", "独立思考", "学术诚信"],
        ...
        return AIResponse(
            ...
            suggestions=["请重新表述您的问题", "检查网络连接", "稍后重试"]
        )
...
async def get_dashboard_summary(
    ...
    try:
        ...
        return AIResponse(
            response=f"生成仪表板摘要时出现错误：{str(e)}",
            suggestions=["稍后重试", "检查数据完整性"]
        )
...
async def analyze_task_file(
    ...
    try:
        ...
        return AIResponse(
            response=f"分析任务时出现错误：{str(e)}",
            suggestions=["检查任务ID", "稍后重试"]
        )
...
async def get_study_tips(
    ...
    try:
        ...
        return AIResponse(
            response=f"获取学习建议时出现错误：{str(e)}",
            suggestions=["稍后重试", "检查课程信息"]
        )
...
async def start_conversation(
    ...
    try:
        ...
        return AIResponse(
            response=f"开始对话时出现错误：{str(e)}",
            suggestions=["重新表述您的问题", "稍后重试"]
        )
